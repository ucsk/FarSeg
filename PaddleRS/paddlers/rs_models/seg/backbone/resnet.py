...
class ResNet50Encoder(nn.Layer):
    def __init__(self,
        ...
        if pretrained is not None:
            para_state_dict = paddle.load(pretrained)
            ...
            for k in keys:
                if k not in para_state_dict:
                    ...
                    logging.warning("[SKIP] Shape of pretrained params {} doesn't match.(Pretrained: {}, Actual: {})"
                                    .format(k, para_state_dict[k].shape, model_state_dict[k].shape))
                else:
                    model_state_dict[k] = para_state_dict[k]
                    num_params_loaded += 1
            self.resnet.set_dict(model_state_dict)
            logging.info("There are {}/{} variables loaded into {}.".format(
                num_params_loaded,
                len(model_state_dict), self.resnet.__class__.__name__))
